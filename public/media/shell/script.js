...
var shellInfoModal = new Spreader('#shell-info-template', {
	onShow: function() {
		...
		$('.shell-info-left img').attr('src', '/media/shell/pics/' + pData.image);
		...
		this.$container.find('[data-form-id="service-request"]')
			.validit({
				onSuccessSubmit: function(){
					...
					console.log(data);
					$.ajax({
						url: '/requestnew/ServiceRequestToCRM',
						type: 'POST',
						data: data,
						success: function(r){
							if (r === 'Y') {
...
//	console.log(shellData);
	...
	$('.content-form__car-img').attr('src', '/media/shell/cars/' + shellData[carNum].image + '.png');
...
function refreshOilList()
{
	...
	for(i = 1; i<=N-1; i++)
	{
		...
		$('.content-formresult .content-inner').append('<div class="formresult-row" data-role="shell-info" data-popup="' + i + '"><div class="formresult-row-number">' + k + '</div><div class="formresult-row-image"><img src="/media/shell/pics/' + popupData[i].image + '"><div class="formresult-row-best' + popupData[i].best + ' ">Лучший выбор</div></div><div class="formresult-row-data"><div class="formresult-row-data-header">' + popupData[i].title + '</div><div class="formresult-row-data-subheader"></div><div class="formresult-row-data-text"><div class="formresult-row-data-textblock">Разработано для бензиновых<br/>и дизельных двигателей</div><div class="formresult-row-data-volume"><div class="volume-header">Объем</div><div class="volume-value">' + shellData[carNum].engines[engineNum].volume + ' л</div></div></div></div></div>');
		k++;
	}
}
