...
$(document).ready(function() {
  ...
  if ( $('.slider_wrap').length ) {
    var nSlider = new Swiper('.nSlider-swiper-container', {
      speed: 900,
      slidesPerView: 'auto',
      centeredSlides: true,
      pagination: {
      ...
      navigation: {
        nextEl: '.swiper-nSlider-navi .swiper-button-next',
        prevEl: '.swiper-nSlider-navi .swiper-button-prev',
      }
    });
  
    var nSliderNav = new Swiper('.nSlider-nav-container', {
      speed: 900,
      slidesPerView: 7,
      navigation: {
        nextEl: '.nSlider-nav .swiper-button-next',
        prevEl: '.nSlider-nav .swiper-button-prev'
      },   
      breakpoints: {
        1262: {
          slidesPerView: 5
    ...
    var nextBtn = $('.nSlider-nav .swiper-button-next');
    var prevBtn = $('.nSlider-nav .swiper-button-prev');
  
    nSlider.on('slideChange', function () {
      nSliderNav.slideTo(nSlider.activeIndex);
      $('.preview-div').removeClass('preview-div--active');
      $('.preview-div').eq(nSlider.activeIndex).addClass('preview-div--active'); 
      $('.preview-slide').eq(nSlider.activeIndex).children().addClass('preview-div--active');
      if ( $('.preview-div--active').parent().next().is('div') ) {
        nextBtn.removeClass('swiper-button-disabled');      
      } else {
        nextBtn.addClass('swiper-button-disabled');
      }
      if ( $('.preview-div--active').parent().prev().is('div') ) {
    ...
    nSlider.on('slideChangeTransitionStart', function() {
      $('.swiper-nSlider-navi .swiper-button-next, .swiper-nSlider-navi .swiper-button-prev').addClass('no-visible');
    });
    
    nSlider.on('slideChangeTransitionEnd', function() {
      $('.swiper-nSlider-navi .swiper-button-next, .swiper-nSlider-navi .swiper-button-prev').removeClass('no-visible');
    });
  
    $('.preview-img').on('click', function(e) {
      var indexSlide = $(this).attr('data-img');
      nSlider.slideTo(indexSlide);
      $('.preview-div').removeClass('preview-div--active');
      $(this).parent().addClass('preview-div--active');
      if ( $('.preview-div--active').parent().next().is('div') ) {
        nextBtn.removeClass('swiper-button-disabled');      
      } else {
        nextBtn.addClass('swiper-button-disabled');
