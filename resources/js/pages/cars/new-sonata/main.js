...
$(function () {
	...
	//Анимация смены системы безопасности
	var animP = new TimelineMax({ repeat: -1, yoyo: true, repeatDelay: 2.5 });

	animP
		.staggerFromTo('.safety .safety__center-img--change-1', 0.5, {
			opacity: 1
		}, {
			opacity: 0
		},	5);

	var animP2 = new TimelineMax({ repeat: -1, yoyo: true, repeatDelay: 2.5 });

	animP2
		.staggerFromTo('.safety .safety__center-img--change-2', 0.5, {
			opacity: 0
		}, {
			opacity: 1
		},	5);

	//Анимация смены приборной панели
	...
	animS
		.staggerFrom('.schema__animate', 0.2, {
			visibility: 'hidden'
		}, 0.2);
	...
	counterAnim
		.add('start')
		.to(count1, 2, { score: '+=180', roundProps: 'score', onUpdate: count1.update.bind(count1), ease: Linear.easeNone }, 'start')
		.to(count2, 2, { score: '+=9', roundProps: 'score', onUpdate: count2.update.bind(count2), ease: Linear.easeNone }, 'start');
