...
$(function () {
	...
	var animP = new TimelineMax({repeat:-1, yoyo:true, repeatDelay: 3})
	...
	//Анимация смены изображения салона
	var animC = new TimelineMax({repeat:-1, yoyo:true, repeatDelay: 2.5})
	...
	counterAnim
		...
		.to(count2, 2, { score: "+=11", roundProps: "score", onUpdate: count2.update.bind(count2), ease: Linear.easeNone }, 'start')

	counterAnim2
		.add('start')
		.to(count3, 2, { score: "+=177", roundProps: "score", onUpdate: count3.update.bind(count3), ease: Linear.easeNone }, 'start')
		.to(count4, 2, { score: "+=9", roundProps: "score", onUpdate: count4.update.bind(count4), ease: Linear.easeNone }, 'start')

	counterAnim3
		.add('start')
		.to(count5, 2, { score: "+=185", roundProps: "score", onUpdate: count5.update.bind(count5), ease: Linear.easeNone }, 'start')
		.to(count6, 2, { score: "+=9", roundProps: "score", onUpdate: count6.update.bind(count6), ease: Linear.easeNone }, 'start')
	...
	$('.js-change-view').on('click', function (e) {
		...
		if (view === 'view0') {
			$('.dynamics__tab').removeClass('dynamics__tab--left dynamics__tab--right');
			$('.dynamics__tab:nth-child(2)').addClass('dynamics__tab--right');
		}
		...
		if (view === 'view2') {
			counterAnim3.play()
			$('.dynamics__tab').removeClass('dynamics__tab--left dynamics__tab--right');
			$('.dynamics__tab:nth-child(2)').addClass('dynamics__tab--left');
		}
	})
	...
	$('.js-toggle-tags').on('click', function(e) {
		e.preventDefault()

		$(this).next('.tags__dd').slideToggle()
	})
