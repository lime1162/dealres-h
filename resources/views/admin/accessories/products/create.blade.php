	...
	<table class="table">
		<caption>
			<div class="table__flex table__flex--caption">
				<h2 class="h2">Добавить аксессуар</h2>
			</div>
		</caption>
	</table>

	<form method="post" action="{{ route('admin.accessories_products.store') }}" enctype="multipart/form-data">
		@csrf
		<div class="content__line">
			{{ Form::adminDropdown('category_id', $item->category_id, ['values' => $categories, 'publicName' => 'Категория', 'placeholder' => 'категорию']) }}
		</div>
		<div class="content__line">
		...
		<div class="content__line">
			{{ Form::adminFile('image', '', ['publicName' => 'Картинка']) }}
		</div>
		...
		@foreach($cars as $car)
			<div class="content__line content__line--2">
				...
				{{ Form::adminInput("car_price[$car->id]", 0, ['placeholder' => 'Введите значение', 'publicName' => '']) }}
			</div>
		@endforeach
		</div>
		<div class="content__line">
