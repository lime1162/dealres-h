	...
	<table class="table">
		<caption>
			<div class="table__flex table__flex--caption">
				<h2 class="h2">Бренд-коллекция: продукция {{ $item->id }}</h2>
			</div>
		</caption>
	</table>

	<form method="post" action="{{ route('admin.brandcollection_products.update', $item->id) }}" enctype="multipart/form-data">
		@method('PATCH')
		@csrf
		...
		<div class="content__line">
			{{ Form::adminFile('image', $item->getImageUrl(), ['publicName' => 'Изображение']) }}
		</div>
		<div class="content__line">
