...
<table class="table">
    <caption>
        <div class="table__flex table__flex--caption">
            <h2 class="h2">Список csv</h2>
            <a href="{{ route('admin.csvfile.create') }}" class="button button--small button--iconed">
                <span>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ $item->name }}</td>
            <td>{{ $item->code }}</td>
            <td><a href="{{ $item->getFileUrl() }}">скачать</a></td>
            <td>
                <div class="table__flex">
                    <div class="table__text"></div>
                    <ul class="table__controls">
                        <li>
                            <a href="{{ route('admin.csvfile.show', $item->id)}}" class="table__link">Изменить</a>
                        </li>
                        <li>
                            <form method="post" action="{{ route('admin.csvfile.destroy', $item->id) }}">
                                @method('DELETE')
                                @csrf
