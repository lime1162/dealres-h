	...
	<table class="table">
		<caption>
			<div class="table__flex table__flex--caption">
				<h2 class="h2">Дилерское меню - категории</h2>
				<a href="{{ route('admin.dealer_menu_categories.create') }}" class="button button--small button--iconed">
					<span>
				...
				<td>
					<div class="table__flex">
						<ul class="table__controls">
							<li>
								<a href="{{ route('admin.dealer_menu_categories.show', $item->id) }}" class="table__link">Изменить</a>
							</li>
							<li>
								<form method="post" action="{{ route('admin.dealer_menu_categories.destroy', $item->id) }}">
									@method('DELETE')
									@csrf
