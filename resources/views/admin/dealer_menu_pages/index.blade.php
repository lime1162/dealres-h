	...
	<table class="table">
		<caption>
			<div class="table__flex table__flex--caption">
				<h2 class="h2">Дилерское меню - страницы</h2>
				<a href="{{ route('admin.dealer_menu_pages.create') }}" class="button button--small button--iconed">
					<span>
		...
		<thead>
			<tr>
				...
				<th data-sort-name="active">Видимость</th>
				<th></th>
			</tr>
		</thead>
		<tbody>
			...
			<tr>
				<td>{{ $item->id }}</td>
				<td>{{ $item->name }}</td>
				<td>{{ ($item->category()->exists()) ? $item->category->name : '' }}</td>
				<td>{{ $item->sort }}</td>
				<td>{{ $item->active > 0 ? 'Виден' : 'Скрыт' }}</td>
				<td>
					<div class="table__flex">
						<ul class="table__controls">
							<li>
								<a href="{{ route('admin.dealer_menu_pages.show', $item->id) }}" class="table__link">Изменить</a>
							</li>
							<li>
								<form method="post" action="{{ route('admin.dealer_menu_pages.destroy', $item->id) }}">
									@method('DELETE')
									@csrf
