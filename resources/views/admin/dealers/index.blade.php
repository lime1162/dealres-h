	...
	<table class="table">
		<caption>
			<div class="table__flex table__flex--caption">
				<h2 class="h2">Дилеры</h2>
				<a href="{{ route('admin.dealers.download_xls') }}" class="button button--small button--iconed">
					Выгрузить
				</a>
				<a href="{{ route('admin.dealers.create') }}" class="button button--small button--iconed">
					<span>
			...
			<tr>
				...
				<td>
					<div class="table__flex">
						...
						<ul class="table__controls">
							<li>
								<a href="{{ route('admin.dealers.show', ['banner' => $item->id]) }}" class="table__link">Изменить</a>
							</li>
							<li>
								<form method="post" action="{{ route('admin.dealers.destroy', $item->id) }}">
									@method('DELETE')
									@csrf
