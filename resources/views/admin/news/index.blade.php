	...
	<table class="table">
		<caption>
			<div class="table__flex table__flex--caption">
				...
				<a href="{{ route('admin.news.create') }}" class="button button--small button--iconed">
					<span>
		...
		<tbody>
			...
			<tr>
				<td>{{ $item->id }}</td>
				<td align="center">{{ $item->dateAdminFormat }}</td>
				<td>{{ $item->title }}</td>
				...
				<td>
					<div class="table__flex">
						...
						<ul class="table__controls">
							<li>
								<a href="{{ route('admin.news.edit', $item->id) }}" class="table__link">Изменить</a>
							</li>
							<li>
