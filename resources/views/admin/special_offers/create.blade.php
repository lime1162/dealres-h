	...
	<table class="table">
		<caption>
			<div class="table__flex table__flex--caption">
				<h2 class="h2">Спец. предложения создание</h2>
			</div>
		</caption>
	</table>

	<form method="post" action="{{ route('admin.special_offers.store') }}" enctype="multipart/form-data">
		@csrf
		...
		<div class="content__line">
			{{ Form::adminFile('image_preview', '', ['publicName' => 'Картинка анонса (465 x 330 px)']) }}
		</div>
		<div class="content__line">
			{{ Form::adminFile('image_world', '', ['publicName' => 'Картинка для Мира Хендэ (1920 x 530)']) }}
		</div>
		<div class="content__line">
			{{ Form::adminFile('banner', '', ['publicName' => 'Баннер (1504 x 545 px)']) }}
		</div>
		<div class="content__line">
			{{ Form::adminFile('banner_mobile', '', ['publicName' => 'Баннер мобильный (640 x 800 px)']) }}
		</div>
		<div class="content__line">
		...
		@foreach($cars as $car)
			<div class="content__line">
				{{ Form::adminCheckbox('car_selected[]', 0, ['id' => 'visible', 'publicName' => $car->name, 'value' => $car->id]) }}
			</div>
		@endforeach
		</div>
		<div class="content__line">
