	...
	<table class="table">
		<caption>
			<div class="table__flex table__flex--caption">
				<h2 class="h2">Баннер {{ $item->id }}</h2>
			</div>
		</caption>
	</table>

	<form method="post" action="{{ route('admin.banners.update', $item->id) }}" enctype="multipart/form-data">
		@method('PATCH')
		@csrf
		...
		<div class="content__line">
			{{ Form::adminFile('image', $item->getImageUrl(), ['publicName' => 'Картинка основная (1504 x 545 px)']) }}
		</div>
		...
		<div class="content__line">
			{{ Form::adminInput('link', $item->link, ['placeholder' => 'Введите текст', 'publicName' => 'Ссылка на баннере']) }}
		</div>
		<div class="content__line">
...
@section('content')
	<table class="table">
		<caption>
			<div class="table__flex table__flex--caption">
				<h2 class="h2">Фото {{ $item->id }}</h2>
			</div>
		</caption>
	</table>

	<form method="post" action="{{ route('admin.storage_photos.update', $item->id) }}" enctype="multipart/form-data">
		@method('PATCH')
		@csrf
		<div class="content__line">
