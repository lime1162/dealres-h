	...
	<table class="table">
		<caption>
			<div class="table__flex table__flex--caption">
				<h2 class="h2">Гарантии на автомобили</h2>
				<a href="{{ route('admin.warranty_cars.create') }}" class="button button--small button--iconed">
					<span>
		...
		<thead>
			<tr>
				...
				<th data-sort-name="is_active">Показывается</th>
				<th></th>
			</tr>
		</thead>
		<tbody>
				...
				<td>
					<div class="table__flex">
						...
						<ul class="table__controls">
							<li>
								<a href="{{ route('admin.warranty_cars.show', $item->id) }}" class="table__link">Изменить</a>
							</li>
							<li>
								<form method="post" action="{{ route('admin.warranty_cars.destroy', $item->id) }}">
									@method('DELETE')
									@csrf
